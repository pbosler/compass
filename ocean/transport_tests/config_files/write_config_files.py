...
all_test_names = ["correlatedTracers2D", "deformation3D", "divergent2D", "Hadley3D",
...
def test_case_id(tcname):
  if tcname == "rotation2D":
    return 1
  elif tcname == "nondivergent2D":
    return 2
  elif tcname == "divergent2D":
    return 3
  elif tcname == "correlatedTracers2D":
    return 4
  else:
    return 0
...
def verify_cfg_input(cfg):
  ...
  for tc in cfg["test_names"]:
    if not tc in all_test_names:
      raise RuntimeError("Invalid test name: " + tc)
...
def get_sed_exp(dt, nprocs, integrator, tcname):
  """
  ...
  tcs = r"s/TMP_flow/" + str(test_case_id(tcname)) + "/g"
  return r"{} ; {} ; {} ; {}".format(dts, nps, tis, tcs)
...
if __name__ == '__main__':
  ...
  for reskey in cfg["resolutions"]:
    ...
    for tc in cfg["test_names"]:
      sedin = get_sed_exp(dt, nprocs, itr, tc)
